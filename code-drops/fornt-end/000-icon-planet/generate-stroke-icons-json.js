 ...
 * ----------------------
 * 自动扫描 ./stroke-icons 文件夹下的所有 .svg 文件
 * 根据文件名分词 + 内置同义词字典，生成一个 stroke-icons.json
 * 
 * 使用方法：
 *   1. 在终端进入本脚本所在目录
 *   2. 运行:  node generate-icons-json.js
 *   3. 程序会在同目录下生成/覆盖 stroke-icons.json
 */
...
// ============== 2. 配置参数  =============
// 图标所在文件夹
const ICONS_DIR = './stroke-icons';
// 输出的 JSON 文件名
const OUTPUT_JSON = './stroke-icons.json';
...
function main() {
  ...
  const iconList = files.map(filename => {
    ...
    return {
      ...
      "path": `../000-icon-planet/stroke-icons/${filename}`,
      // category 可自己补
      "category": "",
      // license 默认填
